/// <reference types="@workadventure/iframe-api-typings" />

import { bootstrapExtra } from "@workadventure/scripting-api-extra";

console.log('Script started successfully');

let currentPopup: any = undefined;
let noteWebsite: any = undefined;

// Waiting for the API to be ready
WA.onInit().then(() => {
    console.log('Scripting API ready');
    console.log('Player tags: ', WA.player.tags);

    WA.room.area.onEnter('clock').subscribe(() => {
        const today = new Date();
        const time = today.getHours() + ":" + today.getMinutes();
        currentPopup = WA.ui.openPopup("clockPopup", "It's " + time, []);
    });

    WA.room.area.onLeave('clock').subscribe(closePopup);

    // The line below bootstraps the Scripting API Extra library that adds a number of advanced properties/features to WorkAdventure
    bootstrapExtra().then(() => {
        console.log('Scripting API Extra ready');
    }).catch(e => console.error(e));

    WA.room.onEnterLayer("visibleNote").subscribe(async () => {
        console.log("Entering visibleNote layer");

        try {
            noteWebsite = await WA.ui.website.open({
                url: "src/note.html",
                position: {
                    vertical: "top",
                    horizontal: "middle",
                },
                size: {
                    height: "30vh",
                    width: "50vw",
                },
                margin: {
                    top: "10vh",
                },
                allowApi: true,
            });
            console.log("noteWebsite opened:", noteWebsite);
        } catch (error) {
            console.error("Error opening noteWebsite:", error);
        }
    });

    WA.room.onLeaveLayer("visibleNote").subscribe(() => {
        if (noteWebsite) {
            console.log("Closing noteWebsite");
            noteWebsite.close().then(() => {
                console.log("noteWebsite closed");
                noteWebsite = undefined;
            }).catch(error => {
                console.error("Error closing noteWebsite:", error);
            });
        } else {
            console.log("noteWebsite is not defined");
        }
    });

    function closePopup() {
        if (currentPopup !== undefined) {
            currentPopup.close();
            currentPopup = undefined;
        }
    }
}).catch(e => console.error(e));

export {};
